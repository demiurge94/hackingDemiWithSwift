...
class DetailViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(shareTapped)) //# tells compiler that the method WILL exist
    ...
    @objc func shareTapped(){
        ...
        let imageName: String = selectedImage!
        let vc = UIActivityViewController(activityItems: [image, imageName],applicationActivities: [])
        vc.popoverPresentationController?.barButtonItem = navigationItem.rightBarButtonItem
        present(vc, animated: true)
        //UIActivityController is the iOS method of sharing content with other apps and services
    }
